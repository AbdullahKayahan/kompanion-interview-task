...
const HomePage = ({route, navigation}) => {
  ...
  useEffect(() => {
    loadData({ token : route.params.token})
      .then(data => setListData({
        data : {...data.posts,isPaused : item.content.type === 'video' ? true : undefined}
      }));
  },[]);
  ...
  useEffect(() => {
    if(changedItems){
      setListData(
        listData.map(_item=>{
          if(_item.data.content.type==="video" && changedItems.findIndex( e=> e.ke === _item.data.id)!=-1) {
            _item.isPausedStates
          }
        })
      )
    }

...
const HomePage = ({route, navigation}) => {
  ...
  useEffect(() => {
    loadData({ token : route.params.token })
      .then(data => setListData(data.posts.map(item => {
        return {
          ...item,
  ...
  useEffect(() => {
    if(changedItems){
      setListData(
        listData.map(_item=>{
          const currentItemIndex = changedItems.findIndex( e=> e.key === _item.id);
          if(_item.content.type==="video" && currentItemIndex !== -1) {
            return {
              ..._item,
              isPaused: changedItems[currentItemIndex].isViewable
            };
          } else {
            return _item;
          }
